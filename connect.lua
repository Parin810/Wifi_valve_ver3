...
tmr.alarm(1, 1000, 1, function() 
       ...
       if(wifiAvailable>60) then
        ...
        tmr.alarm(1, 1000, 1, function()                   --checking for mqtt connection 
                    if not c and tries < 60 then 
                        tries = tries + 1    
                        print (tries)-- incrementing the number of tries 
                        dofile('mqttsetup_valve.lua')                      --calling mqtt script 

                        m:connect("192.168.0.129",1883,0,function(conn) 
                                tmr.stop(1)
                                c = true    
    ...
    tmr.alarm(0,15000,1,function()   --complete all activities in 15 seconds else go to sleep
    node.dsleep(sleepTime)
	print ("sleep")
end)
